...
from models.jointformer import JointTransformer, ErrorRefinement
...
def train_worker(rank, addr, port, args):
    ...
    if rank == 0:
        print("==> Total parameters pose prediction: {:.2f}M".format(sum(p.numel() for p in model_pos.parameters()) / 1000000.0))
    ...
    if args.resume or args.evaluate:
        ...
        if path.isfile(ckpt_path):
            ...
            glob_step = ckpt['step']
            lr_now = ckpt['lr']
            model_refine.load_state_dict(ckpt['state_dict'])
            if 'jt_state_dict' in ckpt.keys():
                model_pos.load_state_dict(ckpt['jt_state_dict'])
            ...
            if rank == 0 and not args.evaluate:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train for one epoch
        epoch_loss, lr_now, glob_step = train(train_loader, model_pos, model_refine, criterion, optimizer, args.lr, lr_now,
        ...
        # Evaluate
        if rank == 0:
            error_eval_p1, error_eval_p2 = evaluate(valid_loader, model_pos, model_refine, intermediate=args.intermediate)
        ...
        if error_best is None or error_best > error_eval_p1 and rank == 0:
            ...
            save_ckpt({'epoch': epoch + 1, 'lr': lr_now, 'step': glob_step, 'state_dict': model_refine.state_dict(),
                       'optimizer': optimizer.state_dict(), 'error': error_eval_p1, 'jt_state_dict': model_pos.state_dict()}, ckpt_dir_path, suffix='best')

        if (epoch + 1) % args.snapshot == 0 and rank == 0:
            save_ckpt({'epoch': epoch + 1, 'lr': lr_now, 'step': glob_step, 'state_dict': model_refine.state_dict(),
                       'optimizer': optimizer.state_dict(), 'error': error_eval_p1, 'jt_state_dict': model_pos.state_dict()}, ckpt_dir_path)
...
def train(data_loader, model_pos, model_refine, criterion, optimizer, lr_init, lr_now, step, decay, gamma, max_norm=True, intermediate=False, scheduler=None, visualizer=None):
    ...
    for i, (targets_3d, inputs_2d, _, input_image) in enumerate(data_loader):
        ...
        pred_3d, _, error_3d = model_pos(inputs_2d, input_image)
        pred_3d = pred_3d[-1]
        error_3d = error_3d[-1]
...
def evaluate(data_loader, model_pos, model_refine, intermediate, ttda=False, kps_left=None, kps_right=None, joints_left=None, joints_right=None):
    ...
    for i, (targets_3d, inputs_2d, _, input_image) in enumerate(data_loader):
        ...
        if intermediate:
            ...
            out = model_refine(refine_in)
            if ttda:
                pred_3d_flip, _, error_3d_flip = model_pos(inputs_2d_flip, input_image)
                pred_3d_flip = pred_3d_flip[-1]
                error_3d_flip = error_3d_flip[-1]
                refine_in_flip = torch.cat([inputs_2d_flip, pred_3d_flip, error_3d_flip], dim=2)
                out_flip = model_refine(refine_in_flip)
            ...
            error_3d = error_3d[-1]
            refine_in = torch.cat([inputs_2d, pred_3d, error_3d], dim=2)
            outputs_3d = model_refine(refine_in).cpu()
            ...
            if ttda:
                ...
                error_3d_flip = error_3d_flip[-1]
                refine_in_flip = torch.cat([inputs_2d_flip, pred_3d_flip, error_3d_flip], dim=2)
                outputs_3d_flip = model_refine(refine_in_flip).cpu()
                outputs_3d_flip[:, :, :] -= outputs_3d_flip[:, :1, :]
                outputs_3d_flip[:, :, 0] *= -1
                outputs_3d_flip[:, joints_left + joints_right] = outputs_3d_flip[:, joints_right + joints_left] 
                outputs_3d = torch.mean(torch.cat([outputs_3d.unsqueeze(0), outputs_3d_flip.unsqueeze(0)], dim=0), dim=0)
